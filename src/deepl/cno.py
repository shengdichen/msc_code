...
from src.deepl.extension.filtered_lrelu import filtered_lrelu
...
class UpFIRDn2d(nn.Module):
    ...
    def __init__(
        self, channel, lrelu_upsampling=2, filter_size=6, gain=np.sqrt(2), slope=0.2
    ):
        super().__init__()
    ...
    def init_kernel(self, in_size, out_size):
        ...
        up_factor = int(
            np.ceil(max(in_size, out_size) * self.lrelu_upsampling / in_size)
        )
        dn_factor = int(
            np.ceil(max(in_size, out_size) * self.lrelu_upsampling / out_size)
        )
        ...
        up_filter = torch.tensor(
            scipy.signal.firwin(
                numtaps=uptaps, cutoff=in_size / 2.0001, width=in_size, fs=in_size
            )
        )
        dn_filter = torch.tensor(
            scipy.signal.firwin(
                numtaps=dntaps, cutoff=out_size / 2.0001, width=out_size, fs=out_size
            )
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        B, C, H, W = x.shape
        assert (
            H == W and H == self.in_size
        ), f"input tensor size({H}) must be equal to in_size({self.in_size})"
        ...
        _, _, H, W = x.shape
        assert (
            H == W and H == self.out_size
        ), "output tensor size must be equal to out_size"
...
class UpFIRDn2dJIT(nn.Module):
    ...
    def init_kernel(self, in_size, out_size):
        ...
        up_filter = scipy.signal.firwin(
            numtaps=uptaps, cutoff=in_size / 2.0001, width=in_size, fs=in_size
        )
        dn_filter = scipy.signal.firwin(
            numtaps=dntaps, cutoff=out_size / 2.0001, width=out_size, fs=out_size
        )
        ...
        pad_total = (
            out_size - 1
        ) * self.dn_factor + 1  # Desired output size before downsampling
        pad_total -= (
            in_size + self.kernel_size - 1
    ...
    def forward(self, x):
        if x.device.type == "cuda":
            ...
            warnings.warn(
                "filtered_lrelu_jit is not supported on CPU. Use filtered_lrelu instead."
            )
            ...
            assert (
                H == W and H == self.in_size
            ), f"input tensor size({H},{W}) must be equal to in_size({self.in_size})"
            ...
            x = F.conv2d(
                x,
                torch.outer(self.up_filter, self.up_filter)[None, None, :, :],
                padding="same",
            )
            x = F.leaky_relu(x, negative_slope=self.slope) * self.gain
            x = F.conv2d(
                x,
                torch.outer(self.dn_filter, self.dn_filter)[None, None, :, :],
                padding="same",
            )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if jit:
            ...
            self.conv = nn.Conv2d(
                in_channel, out_channel, kernel_size, stride=1, padding="same"
            )
...
class ResBlock2d(nn.Module):
    ...
    def __init__(
        ...
        self.convs = nn.ModuleList(
            [Conv2d(in_channel, out_channel, kernel_size, activation=activation)]
        )
        for _ in range(num_layers - 2):
            self.convs.append(
                Conv2d(out_channel, out_channel, kernel_size, activation=activation)
            )
        self.convs.append(
            Conv2d(out_channel, out_channel, kernel_size, activation=None)
        )
...
class DownBlock2d(nn.Module):
    def __init__(
        ...
        self.conv_next = Conv2d(
            in_channel, in_channel, kernel_size, activation=activation
        )
        self.conv_down = Conv2d(
            in_channel,
            self.output_channel,
            kernel_size,
            stride=scale_factor,
...
class UpBlock2d(nn.Module):
    def __init__(
        ...
        self.conv_next = Conv2d(
            2 * out_channel, out_channel, kernel_size, activation=activation
        )
        self.reset_parameters()
...
class GeneralUNet2d(nn.Module):
    ...
    def __init__(
        ...
        self.input_transform = Conv2d(
            in_channel, hidden_channel, kernel_size, activation=activation
        )  # lift
        self.output_transform = Conv2d(
            hidden_channel, out_channel, kernel_size, activation=None
        )  # project
