...
def exact_solution(d: float, w0: float, t: torch.Tensor) -> np.ndarray:
    if d >= w0:
        raise ValueError
...
class NetworkFullConnect(nn.Module):
    def __init__(
        self, size_input, size_output, size_hidden, n_layers: int, activation=nn.Tanh
    ):
        super().__init__()
...
class Pinn:
    def __init__(self):
        ...
        self._network = NetworkFullConnect(1, 1, 32, 3)
        ...
        d, w0 = 2, 20
        self._constant_mu, self._constant_k = 2 * d, w0**2
        self._t_test = torch.linspace(0, 1, 300).view(-1, 1)
        self._u_exact = exact_solution(d, w0, self._t_test)
